...
extern double darkOmegaExt(double *Xf, double (*f0)(double),
                           double (*f1)(double));
extern double darkOmegaTR(double TR, double YR, int Fast, double Beps,
                          int *err);
extern double darkOmega2TR(double TR, double Y1R, double Y2R, double fast,
                           double Beps);
...
/*
 calculates the spectra of DM annihilation at rest and returns σv in cm^3/s. The
 calculated spectra for γ, e + , p̄, ν_e , ν_μ , ν_τ are stored in arrays Sg, Se,
 Sp, Sne, Snm, Snl  of dimension NZ(=250 by default).

 key =n1+2*n2+4*n4    is a bit switch where n_i is 1 or zero
 n1=1  switch on effect of W,Z polarization
 n2=1  includes photon radiation for DM annihilation into light fermions (2->3
 process) n4=1  prints contribution of different annihilation channels to σv
*/

extern void decaySpectrum(char *pName, int outP, double *tabD);
extern void getSpectrum2(int wPol, double M, char *n1, char *n2, int outP,
                         double *tab);
...
extern double captureCS(double (*vfv)(double), int forSun, double M_cdm,
...
extern double maxRecoil(double A); // calculate maximal recoil energy using
                                   // atomic numner A, Mcdm1,Mcdm2,Mcdm
...
extern double nucleusRecoilCS(double (*vfv)(double), int A, int Z, double J,
                              void (*Sxx)(double, double *, double *, double *),
                              double cs_SI_P, double cs_SI_N, double cs_SD_P,
                              double cs_SD_N, double *dNdE);

extern double nucleusRecoil0CS(double (*vfv)(double), int A, int Z, double J,
...
extern double DD_factorCS(unsigned int Experiment, double pval,
                          double (*vfv)(double), double cs_SI_P, double cs_SI_N,
                          double cs_SD_P, double cs_SD_N, const char **expName);
extern double DD_pvalCS(unsigned int Experiment, double (*vfv)(double),
                        double cs_SI_P, double cs_SI_N, double cs_SD_P,
                        double cs_SD_N, const char **expName);

extern double DD_factor(unsigned int Experiment, double pval,
                        double (*vfv)(double), const char **expName);
extern double DD_pval(unsigned int Experiment, double (*vfv)(double),
                      const char **expName);
...
extern int smodels(int Run, int nf, double csMinFb, char *fileName,
                   char *version, int wrt);
...
extern SxxType SxxF19EFT, SxxXe131EFT, SxxXe129EFT, SxxF19SHELL, SxxXe129SHELL,
    SxxXe131SHELL, SxxF19SHELLm, SxxXe129SHELLm, SxxXe131SHELLm;
