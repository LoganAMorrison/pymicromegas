 ...
 */
#define CDM_NUCLEUS
// Calculate  exclusion rate for direct detection experiments Xenon1T and
// DarkSide50
...
#include "../include/SMODELS.inc" // SLHA interface with SModelS
    ...
    if (smodelsOK) {
      ...
      system("cat smodels.err"); // problem: see smodels.err
  }
...
#ifdef CDM_NUCLEON
  {
    ...
    if (pA5[0] * nA5[0] < 0)
      ...
      sD = 1;
    const char *expName = "";
    double pval = DD_pvalCS(AllDDexp, Maxwell, csSIp, sI * csSIn, csSDp,
                            sD * csSDn, &expName);
    if (pval < 0.1)
      printf("Excluded by %s [CDM_NUCLEON] %.1f%% \n", expName,
             100 * (1 - pval));
    else
...
#ifdef CDM_NUCLEUS
  {
    ...
    if (pval < 0.1)
      printf("Excluded by %s [CDM_NUCLEUS]  %.1f%%\n", expName,
             100 * (1 - pval));
    else
      printf("Not excluded by DD experiments  at 90%% level \n");
  }
