...
void rungeKuttaStep(const DoubleVector &y, const DoubleVector &dydx, double x,
                    double h, DoubleVector &yout, DoubleVector &yerr,
                    DoubleVector (*derivs)(double, const DoubleVector &));

/// organises the variable step-size for Runge-Kutta evolution
int odeStepper(DoubleVector &y, const DoubleVector &dydx, double *x,
               double htry, double eps, DoubleVector &yscal, double *hdid,
               double *hnext,
               DoubleVector (*derivs)(double, const DoubleVector &));
...
int integrateOdes(DoubleVector &ystart, double x1, double x2, double eps,
                  ...
                  DoubleVector (*derivs)(double, const DoubleVector &),
                  int (*rkqs)(DoubleVector &y, const DoubleVector &dydx,
                              double *x, double htry, double eps,
                              DoubleVector &yscal, double *hdid, double *hnext,
                              DoubleVector (*derivs)(double,
                                                     const DoubleVector &)));
...
/// function
double calcDerivative(double (*func)(double, void *), double x, double h,
                      double *err, void *params = NULL);
...
inline double ffn(double p, double m1, double m2, double q) {
  return a0(m1, q) - 2.0 * a0(m2, q) -
         (2.0 * sqr(p) + 2.0 * sqr(m1) - sqr(m2)) * b0(p, m1, m2, q);
}
/// inlined PV functions: real part
...
inline Complex ffnc(double p, double m1, double m2, double q) {
  return a0(m1, q) - 2.0 * a0(m2, q) -
         (2.0 * sqr(p) + 2.0 * sqr(m1) - sqr(m2)) * b0c(p, m1, m2, q);
}
/// inlined PV functions: complex answer
...
void polint(const DoubleVector &xa, const DoubleVector &ya, double x, double &y,
            double &dy);
...
void getAngles(const DoubleMatrix &v, double &t12, double &t13, double &t23,
               double &d);
...
bool lnsrch(const DoubleVector &xold, double fold, const DoubleVector &g,
            DoubleVector &p, DoubleVector &x, double &f, double stpmax,
            int (*vecfunc)(const DoubleVector &, void *, DoubleVector &),
...
void qrdcmp(DoubleMatrix &a, int n, DoubleVector &c, DoubleVector &d,
            int &sing);
void qrupdt(DoubleMatrix &r, DoubleMatrix &qt, int n, DoubleVector &u,
            DoubleVector &v);
...
/// factor
/// \f$
/// f(m_1,m_2,m_3)=\frac{\sqrt{(m_1^2-(m_2^2+m_3^2))(m_1^2-(m_2^2-m_3^2))}}{2
/// m_1}\f$.
